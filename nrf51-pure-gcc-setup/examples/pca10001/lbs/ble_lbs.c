...
void ble_lbs_on_ble_evt(ble_lbs_t * p_lbs, ble_evt_t * p_ble_evt) {
	...
	case BLE_GAP_EVT_CONNECTED:
		uart_tx_str("on_connect");
		on_connect(p_lbs, p_ble_evt);
		break;
	case BLE_GAP_EVT_DISCONNECTED:
		uart_tx_str("on_disconnect");
		on_disconnect(p_lbs, p_ble_evt);
		break;
	case BLE_GATTS_EVT_WRITE:
		uart_tx_str("on_write");
		on_write(p_lbs, p_ble_evt);
		break;
	default:
		break;
	}
	uart_tx_status_code("p_ble_evt->header.evt_id =", p_ble_evt->header.evt_id,
			UART_HEX_MODE);
}
...
static uint32_t led_char_add(ble_lbs_t * p_lbs,
	...
	return sd_ble_gatts_characteristic_add(p_lbs->service_handle, &char_md,
			&attr_char_value, &p_lbs->led_char_handles);
}
...
static uint32_t button_char_add(ble_lbs_t * p_lbs,
	...
	attr_char_value.max_len = sizeof(uint8_t);
//attr_char_value.p_value = NULL;
	attr_char_value.p_value = &init_button_value;
	return sd_ble_gatts_characteristic_add(p_lbs->service_handle, &char_md,
			&attr_char_value, &p_lbs->button_char_handles);
}
